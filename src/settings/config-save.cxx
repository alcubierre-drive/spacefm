...
xset_pack_set(const xset_t& set)
{
    ...
    if (set->menu_label)
    {
        ...
        { // built-in
            ...
            { // only save lbl if menu_label was customized
                ...
                const auto value = fmt::format("{}", set->menu_label.value());
                setvars.insert({name.data(), value});
            }
        }
        else
    ...
    { // built-in
        ...
        { // only save icn if icon was customized
            ...
            const auto value = fmt::format("{}", set->icon.value());
            setvars.insert({name.data(), value});
        }
    }
    else if (set->icon)
